    ...
    public class EaiLauncher : BaseStationFactory
    {
        public override async Task SendAsync(PayloadRoot root, HostChannel channel)
        {
            try
            {
                ...
                //Console.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff")+" [sMMP => EAI] Request:\r\n" + xRoot.ToString());
                LogBuilder.WriteLog(LogEventLevel.Information, "[P3] [sMMP => EAI] Request:\r\n" + xRoot.ToString());

                var response = await request.invokeSrvAsync(xRoot.ToString());
                string result = response.Body.invokeSrvReturn;
                //Console.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff") + " [EAI => sMMP] Response:\r\n" + result);
                LogBuilder.WriteLog(LogEventLevel.Information, "[P4] [EAI => sMMP] Response:\r\n" + result);
            }
            catch (Exception e)
            {
